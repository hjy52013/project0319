...
//      mTimer.schedule(mTimerTask, 0, 20 * 1000);// 20s
    ...
    //检查wifi是否存在
    public boolean isWifiExsits(String wifiSsid){
        List<ScanResult> scanResultList=mWifiManager.getScanResults();
        for (ScanResult scanResult : scanResultList) {
            if (scanResult.SSID.equals(wifiSsid) /*&& existingConfig.preSharedKey.equals("\"" + password + "\"")*/) {
                return true;
            }
        }
        return false;
